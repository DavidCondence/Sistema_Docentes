...
public class Usuario {
    ...
    public void eliminarUsuario(int idusuario) throws SQLException { 
         eliminarCursosInscrito(idusuario);
         deleteAccount(idusuario);
    }
    public void eliminarCursosInscrito(int idusuario) throws SQLException{
        String sql="SELECT * FROM inscritos WHERE UserID="+idusuario+""; 
        List<BeanUsuario> listaUuarios = new ArrayList<BeanUsuario>(); 
        Conexion c=new Conexion();
        Connection con=c.getConexion();
        Statement statement = con.createStatement();
        ...
        while (resulSet.next()) {
            System.out.println(resulSet.getInt("UserID"));
            Statement st;
            st = con.createStatement();
            st.executeUpdate("DELETE FROM inscritos WHERE UserID="+resulSet.getInt("UserID")+"");  
            st.close();    
        }
        con.close(); 
    }
    public void deleteAccount(int idusuario)throws SQLException {
        ...
        Statement st;
        st = con.createStatement();
        st.executeUpdate("DELETE FROM usuario WHERE UserID="+idusuario+"");  
        st.close();    
        con.close(); 
    }
